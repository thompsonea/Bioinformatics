MachineRead_output <- read.csv("/cloud/project/MachineRead_output.csv") #Import data from dataframe
...
for (each in MachineRead_output$Paper.ID) { #for each in the dataframe under Paper.ID
...
MachineRead_output <- MachineRead_output[-c(rowsToRemove),] #remove LEEs that occured once from dataframe
